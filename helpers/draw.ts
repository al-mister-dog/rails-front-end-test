...
function reOrderParticipants(
  ...
  return participants.map((participant) => {
    const thisParticipant = (p: { email: string }) => p.email === participant.email;

    const santaFor = (p: { email: string }) =>
      p.email === participantEntry[0].santa_for;

    const participantEntry = shuffledParticipants.filter(thisParticipant);

    const recipientId = shuffledParticipants.find(santaFor)?.email;
    return { ...participant, santa_for: recipientId || "" };
  });
}
